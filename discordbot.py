...
ID_CHANNEL_MAIN = 664484186023002128  # 入力用チャンネルID（事前設定用）
ID_CHANNEL_LOG_MAIN = 664484208638558248  # 凸進捗出力用チャンネルID（事前設定用）
ID_CHANNEL_LOG_INCOMPLETE = 664484220147597341  # 3凸未完了者リスト用チャンネルID（事前設定用）
ID_CHANNEL_LOG_REQUEST = 664484234236264448  # 持越中・通知登録者リスト用チャンネルID（事前設定用）
ID_CHANNEL_REACT = 664485172670103585  # 簡易入力用チャンネルID（事前設定用）
# 簡易入力用絵文字ID（物理凸,物理〆,魔法凸,魔法〆）（事前設定用）
ID_EMOJI = [ 618947677404528662, 553960389189042176, 553960390279823380, 618947668735164426, 553960392364130304]
########################################################################################################################
...
async def submit_attack_log(message, orig_user):
    ...
    if re.match(r'^[1-5][物魔bm]', msg_content):
        ...
        if is_finished:
            ...
            # 待機者が居たら通知
            if rolled_target:
                reply += f'{rolled_target} {next_boss}ボスで持越が使えますわ。起きてください。\n'
            if notice_target:
                reply += f'{notice_target} {next_boss}ボスの時間ですわ\n'
            if reply:
